...
def get_feature_importance(model):
    ...
    df = pd.DataFrame(feature_importance, columns)
...
def get_input_predict_analysis(result: ModelResult):
    ...
    predictions = pd.Series(encoders['PLANTAÇÃO'].inverse_transform(predictions))
    lengthiest_plantacao = predictions.str.len().max()

    divisor_plantacao = ' '.join(['' for _ in range(lengthiest_plantacao - len('PLANTAÇÃO') + 2)])

    analysis = f'INVASORA{divisor_invasora}PLANTAÇÃO{divisor_plantacao}PREDIÇÃO'

    for invasora, predicted, result in zip(invasoras, predictions, results):
        divisor_invasora = ' '.join(['' for _ in range(lengthiest_invasora - len(invasora) + 2)])
        divisor_plantacao = ' '.join(['' for _ in range(len('PLANTAÇÃO') - len(predicted) + 4)]) if len(predicted) < len('PLANTAÇÃO') \
            else ' '.join(['' for _ in range(lengthiest_plantacao - len(predicted) + 2)])
        analysis += f'\n{invasora}{divisor_invasora}{predicted}{divisor_plantacao}{result}'
...
if __name__ == '__main__':
    ...
    expected_output = 'PLANTAÇÃO'
    alternative = 'COLETA DA AMOSTRA'
    encoders = {}

    input_data, columns = get_input_data()
    ...
    with open('result/decision_tree_fi_cultura.txt', 'w') as f:
        f.write(f'{get_feature_importance(decision_tree)}')

    with open('result/random_forest_fi_cultura.txt', 'w') as f:
        f.write(f'{get_feature_importance(random_forest)}')

    with open('result/decision_tree_analise_predicao_cultura.txt', 'w') as f:
        f.write(get_input_predict_analysis(decision_tree_result))

    with open('result/random_forest_analise_predicao_cultura.txt', 'w') as f:
        f.write(get_input_predict_analysis(random_forest_result))

    with open('result/knn_analise_predicao_cultura.txt', 'w') as f:
        f.write(get_input_predict_analysis(knn_result))

    with open('result/svm_analise_predicao_cultura.txt', 'w') as f:
